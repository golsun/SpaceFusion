...
class DecoderAR:
	...
	def predict_beam(self, latents, beam_width=10, max_n_hyp=100):
		# multi-head beam search by Xiang Gao
		...
		while True:
			...
			for node in beam:
				...
				for ix in top_tokens[:beam_width]:

					logP = node['logP'] + np.log(tokens_proba[ix])
					hyp = node['hyp'] + ' ' + self.dataset.index2token[ix]
					if ix == self.dataset.EOS:
					...
					if pq.qsize() > beam_width:
						pq.get()	# pop the node of lowest logP to maintain at most beam_width nodes
